...
RelationDefinition.hasMany = function hasMany(modelFrom, modelTo, params) {
  ...
  if(definition.modelThrough) {
    ...
    addFunc.shared = true;
    addFunc.http = {verb: 'post', path: '/' + relationName + '/add'};
    addFunc.accepts = {arg: 'foreignKey', type: 'any', required: true, http: {source: 'query'}};
    addFunc.description = 'Add a related item by id for ' + relationName;
    addFunc.returns = {arg: relationName, type: 'object', root: true};

    modelFrom.prototype['__add__' + relationName] = addFunc;

    var removeFunc = scopeMethods.remove;
    removeFunc.shared = true;
    removeFunc.http = {verb: 'post', path: '/' + relationName + '/remove'};
    removeFunc.accepts = {arg: 'foreignKey', type: 'any', required: true, http: {source: 'query'}};
    removeFunc.description = 'Remove a related item by id for ' + relationName;
    removeFunc.returns = {arg: relationName, type: 'object', root: true};

    modelFrom.prototype['__remove__' + relationName] = removeFunc;
