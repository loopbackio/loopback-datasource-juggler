...
RelationDefinition.embedsOne = function(modelFrom, modelTo, params) {
  ...
  if (definition.options.validate !== false) {
    modelFrom.validateAsync(relationName, function(err, done) {
      var self = this;
      var inst = self[propertyName];
...
RelationDefinition.embedsMany = function embedsMany(modelFrom, modelTo, params) {
  ...
  if (definition.options.validate !== false) {
    modelFrom.validateAsync(propertyName, function(err, done) {
      var self = this;
      var embeddedList = this[propertyName] || [];
      var hasErrors = false;

      async.forEachOf(embeddedList, function(item, idx, eachCb) {
