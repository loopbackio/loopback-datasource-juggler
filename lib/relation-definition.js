...
RelationDefinition.embedsMany = function embedsMany(modelFrom, modelToRef, params) {
  ...
  if (!params.polymorphic) {
    modelFrom.validate(propertyName, function(err) {
      const embeddedList = this[propertyName] || [];
      const idNames = embeddedList.itemType.definition._ids.map(function(id) {
        return id.name;
      });
      const ids = embeddedList.map(function(m) {
        let idList = idNames.map(function(idName) {
      ...
      if (ids.length !== uniqueIds.length) {
        this.errors.add(propertyName, 'contains duplicate `' + idNames + '`', 'uniqueness');
        err(false);
      }
    }, {code: 'uniqueness'});
  }
