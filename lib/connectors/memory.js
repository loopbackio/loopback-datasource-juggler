...
var self = this;
  try {
    ...
    if (response == true) {
      ...
      self.all(filter, function (err, models) {
        ...
        if (err) {
          ...
          var idName = idName(Model);

          if (models.length > 1) {

            callback("Error message:" + "Multiple records exists.Upsert can't be performed.");
          } else if (models.length == 0 && data.hasOwnProperty(idName)) {
          ...
          } else if (models.length == 0 && data.hasOwnProperty(idName) == false) {

            callback("Error message: Primary key is not provided in the payload");
          } else if (models.length ==1){


            var idName = idName(Model);

            var idVal = models[0][idName];
      ...
      callback("Error Message: Data/payload can't be empty");
  } catch (ex) {
