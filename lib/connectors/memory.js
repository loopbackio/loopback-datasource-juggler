...
Memory.prototype.fromDb = function(model, data) {
  ...
  const props = typeof model === 'object' ? model.properties : this._models[model].properties;
  for (const key in data) {
    ...
    if (props[key]) {
      const name = props[key].type.name;
      if (name === 'Date') {
        ...
        val = Number(val);
      } else if (typeof name === 'string' && name.indexOf('AnonymousModel') === 0) {
        if (typeof val === 'object') {
          val = this.fromDb(props[key].type.definition, val);
        }
      }
    }
    data[key] = val;
