...
function applyFilter(filter) {
  ...
  function test(example, value) {
    ...
    if (typeof example === 'object') {
      ...
      if (isTypeSupportComparison(example.gt) && example.gt < value) return true;
      if (isTypeSupportComparison(example.gte) && example.gte <= value) return true;
      if (isTypeSupportComparison(example.lt) && example.lt > value) return true;
      if (isTypeSupportComparison(example.lte) && example.lte >= value) return true;
    }
    // not strict equality
    return (example !== null ? example.toString() : example) == (value !== null ? value.toString() : value);
  }

  function isTypeSupportComparison(n) {
    return isNum(n) || isDate(n);
  }
  function isNum(n) {
