...
/*
 * Coerce values based on the properties types
...
ModelUtils._coerce = function(where, options) {
  ...
  for (const p in where) {
    this._coerceProperty(p, where, options);
  }
  return where;
};

/*
 * Coerce values based on the property type
 ...
 * @private
 */
ModelUtils._coerceProperty = function(p, where, options) {
  const self = this;
  ...
  if (!DataType) {
    if (p.indexOf('.') !== -1) {
      DataType = this._getNestedPropertyDataType(p, props);
    } else {
      return;
    }
  }
...
/*
 * Coerce values based on the nested property type
 * @param property The property
 * @param property[] The model properties
 * @returns DataType The property type
 * @private
 */
ModelUtils._getNestedPropertyDataType = function(p, props) {
  const parts = p.split('.');
  const end = parts.pop();
  let obj = props;
  while (parts.length) {
    const part = parts.shift();
    if (typeof obj[part].type !== 'function' ||
      !obj[part].type.definition ||
      !obj[part].type.definition.properties) {
      return;
    }
    obj = obj[part].type.definition.properties;
  }
  return obj[end] && obj[end].type;
};
