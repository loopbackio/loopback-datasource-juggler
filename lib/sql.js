...
BaseSQL.prototype.save = function (model, data, callback) {
  ...
  var sql = 'UPDATE ' + this.tableEscaped(model) + ' SET '
    + this.toFields(model, data) + ' WHERE ' + this.idColumnEscaped(model) + ' = ' + this.toDatabase(props[this.idColumn(model)], data.id);
...
BaseSQL.prototype.exists = function (model, id, callback) {
  ...
  var sql = 'SELECT 1 FROM ' +
    this.tableEscaped(model) + ' WHERE ' + this.idColumnEscaped(model) + ' = ' + this.toDatabase(props[this.idColumn(model)], id) + ' LIMIT 1';
...
BaseSQL.prototype.delete = BaseSQL.prototype.destroy = function destroy(model, id, callback) {
  var props = this._models[model].properties;
  var sql = 'DELETE FROM ' +
    this.tableEscaped(model) + ' WHERE ' + this.idColumnEscaped(model) + ' = ' + this.toDatabase(props[this.idColumn(model)], id);
