...
ModelBuilder.prototype.define = function defineClass(className, properties, settings, parent) {
  ...
  // // inherit validations
    if (ModelClass.validations && !subClass.settings.realmRequired) {
      // FIXME we need to have better way to remove validations
      // User base model enforces uniqueness on email and username, but
      // realm requires composite key uniqueness which is not yet supported
      subClass.validations = traverse(ModelClass.validations).clone();
    }
