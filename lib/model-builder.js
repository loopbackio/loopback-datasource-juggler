...
var debug = require('debug')('loopback:dao:modelbuilder');
...
ModelBuilder.prototype.define = function defineClass(className, properties, settings, parent) {
  ...
  ModelClass.extend = function(className, subclassProperties, subclassSettings) {
    ...
    debug('Extending %s class from %s', className, ModelClass.modelName);
  ...
  // // inherit validations
    if (ModelClass.validations) {
      inheritValidations(ModelClass, subClass);
    }
...
/*!
 * Used in ModelClass.extend()
 * this logic is separated because validation has option to avoid inheritance via option
 */
function inheritValidations(ModelClass, subClass) {
  if (!subClass.validations)
    hiddenProperty(subClass, 'validations', {});

  for (var prop in ModelClass.validations) {
    var validationCollection = [];
    for (var val in ModelClass.validations[prop]) {
      var validationObj = ModelClass.validations[prop][val];
      if (!validationObj.disableInherit)
        validationCollection.push(validationObj);
    }
    subClass.validations[prop] = validationCollection;
  }
}
