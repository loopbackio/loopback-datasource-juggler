    ...
    targetModel.find(params, options, function(err, data) {
      ...
      if (scopeOnRelatedModel === true) {
        ...
        var smartMerge = function(idCollection, qWhere) {
          ...
          // Return invalid filter like `{where: {id: {inq: 1}}}`, will be caught by validatior
          if (!Array.isArray(qWhere[IdKey].inq)) {
            return qWhere[IdKey];
          }
          ...
          // if the id in filter doesn't exist in related items,
          // return empty result
          if (intersect.length < 1) return cb(null, []);
          delete qWhere[IdKey];
          return merged;
        };
