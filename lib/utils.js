...
function safeRequire(module) {
  try {
    ...
    g.log('Run "npm install {{loopback-datasource-juggler}} ' + module +
    ...
    if (x === undefined) {
      switch (handleUndefined) {
        case 'nullify':
          ...
          break;
        case 'throw':
          throw new Error(g.t('Unexpected {{`undefined`}} in query'));
          break;
        case 'ignore':
        default:
          this.remove();
...
function toRegExp(regex) {
  ...
  if (!(isString || isRegExp))
    return new Error(g.t('Invalid argument, must be a string, {{regex}} literal, or ' +
        '{{RegExp}} object'));
