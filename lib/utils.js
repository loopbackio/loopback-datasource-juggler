...
function applyParentProperty(element, parent) {
  ...
  if (!builderSettings || !builderSettings[BUILDER_PARENT_SETTING]) return; // parentRef flag not enabled on ModelBuilder settings
  const parentPropertyName = '__parent';
  if (element.hasOwnProperty(parentPropertyName)) { // property already created on model, just assign
    const existingParent = element[parentPropertyName];
    if (existingParent && existingParent !== parent) { // parent re-assigned (child model assigned to other model instance)
      g.warn('Re-assigning child model instance to an other parent than the original! ' +
        'Although supported, this is not a recommended practice: ' +
        `${element.constructor.name} -> ${parent.constructor.name}`);
    }
    element[parentPropertyName] = parent;
  } else { // first time defining the property on the element
    Object.defineProperty(element, parentPropertyName, {
      value: parent,
      writable: true,
      enumerable: false,
      configurable: false,
