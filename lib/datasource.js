...
DataSource.prototype.discoverSchemas = function (modelName, options, cb) {
  ...
  if (followingRelations) {
    ...
    tasks.push(this.discoverExportedForeignKeys.bind(this, modelName, options));
  }
  ...
  async.parallel(tasks, function (err, results) {
    ...
    if (followingRelations) {
      ...
      for (var fkIndex = 2; fkIndex < 4; fkIndex++) {
        // Handle foreign keys

        var foreignKeys = results[fkIndex];

        foreignKeys.forEach(function (fk) {
          var tableName = fkIndex == 2 ? fk.pkTableName : fk.fkTableName
          var colName   = fkIndex == 2 ? fk.fkColumnName : fk.pkColumnName

          var propName = fromDBName(tableName, true);
          schema.options.relations[propName] = {
            model: fromDBName(tableName, false),
            type: fkIndex == 2 ? 'belongsTo' : 'hasMany',
            foreignKey: fromDBName(colName, true)
          };

          var key = fk.pkOwner + '.' + tableName;
          if (!options.visited.hasOwnProperty(key) && !otherTables.hasOwnProperty(key)) {
            otherTables[key] = {owner: fk.pkOwner, tableName: tableName};
          }
        });
      }
    }
...
DataSource.prototype.discoverSchemasSync = function (modelName, options) {
  ...
  if (followingRelations) {
    ...
    var foreignKeys = this.discoverExportedForeignKeysSync(modelName, options);

    foreignKeys.forEach(function (fk) {
      var propName = fromDBName(fk.fkTableName, true);
      schema.options.relations[propName] = {
        model: fromDBName(fk.fkTableName, false),
        type: 'belongsTo',
        foreignKey: fromDBName(fk.pkColumnName, true)
      };

      var key = fk.pkOwner + '.' + fk.fkTableName;
      if (!options.visited.hasOwnProperty(key) && !otherTables.hasOwnProperty(key)) {
        otherTables[key] = {owner: fk.pkOwner, tableName: fk.fkTableName};
      }
    });    
