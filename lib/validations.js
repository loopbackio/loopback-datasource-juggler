  ...
  this.trigger('validate', function (validationsDone) {
    ...
    var nestedModelsToValidate = Object.keys(inst.constructor.definition.properties)
      .filter(function (attr) {
        ...
        return inst[attr].isValid === Validatable.prototype.isValid;
      });
    nestedModelsToValidate.forEach(function (attr) {
      var nestedInst = inst[attr];
      var nestedData = data[attr];
      var nestedCallback = function () {};
      var nestedValid = nestedInst.isValid(function (valid) {
      ...
      if (nestedValid === undefined) {
        ...
        nestedCallback = function (fail) {
          if (fail)
            inst.errors.addNested(attr, nestedInst.errors);
          done(fail);
        };
      } else if (!nestedValid) {
        inst.errors.addNested(attr, nestedInst.errors);
