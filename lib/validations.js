...
function validateUniqueness(attr, conf, err, done) {
  ...
  this.constructor.find(cond, function (error, found) {
    ...
    var primaryKeys = this.constructor.definition.idNames();
    if (found.length > 1) {
      err();
    } else if (found.length === 1 && !primaryKeys.every(function(key) { return this[key] && found[0][key] && found[0][key].toString() == this[key].toString(); }, this)) { 
      err();
    }
    done();
  }.bind(this));
