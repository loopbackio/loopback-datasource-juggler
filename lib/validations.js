 ...
 *
 * // MyModel uses a custom primaryKey (not 'id')
 * MyModel.validatesUniquenessOf('property', { primaryKey: 'myModelId' });
 * ```
...
function validateUniqueness(attr, conf, err, done) {
  ...
  this.constructor.find(cond, function (error, found) {
    ...
    var primaryKey = (conf && conf.primaryKey) || 'id';
    if (found.length > 1) {
      err();
    } else if (found.length === 1 && (!this[primaryKey] || !found[0][primaryKey] || found[0][primaryKey].toString() != this[primaryKey].toString())) {
      err();
    }
    done();
  }.bind(this));
