...
DataAccessObject.find = function find(query, options, cb) {
  ...
  if (near) {
    if (supportsGeo) {
      ...
      if (options.notify === false) {
        DataAccessObject.queryGeo(query);
      } else {
        DataAccessObject.withNotifyGeo();
      }
      // already handled
      return cb.promise;
    }
...
// refactored functions from DAO.find
// var cb = cb || utils.createPromiseCallback();
// var query = query || {};
// var options = options || {};
// var hookState = {};

DataAccessObject.withNotifyGeo = function() {
  var cb = cb || utils.createPromiseCallback();
  var query = query || {};
  var options = options || {};
  var hookState = {};
  ...
  this.notifyObserversOf('access', context, function(err, ctx) {
    if (err) return cb(err);
    DataAccessObject.queryGeo(ctx.query);
  });
};

DataAccessObject.queryGeo = function(query) {
  var cb = cb || utils.createPromiseCallback();
  var options = options || {};
  var hookState = {};
  var self = this;
