...
DataAccessObject._coerce = function(where) {
  ...
  for (var p in where) {
    ...
    var val = where[p];
    // if the where query only ask a certain parameter exist or not in the model,
    // don't validate the data type and set it to undefined to make it work
    if (val !== undefined && val !== null) {
      var exists =  val.exists;
