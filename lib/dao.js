...
DataAccessObject.upsertWithWhere = function(where, data, options, cb) {
  ...
  var Model = this;
  var connector = Model.getConnector();
  var modelObject = connector._models;
  var modelName = Object.keys(modelObject)[0];
  if (connector.upsertWithWhere) {
    return connector.upsertWithWhere(modelName, where, data, cb);
  } else {
    var element = Object.keys(where)[0];
    var filter = {
      "where": where
    };
    if (Object.keys(data).length != 0) {
      if (!data.hasOwnProperty(element)) {
        data[element] = where[element];
      }
      var pk = idName(Model);
      self.find(filter, function(err, instances) {
        var modelsLength = instances.length;
        if (modelsLength === 0 && data.hasOwnProperty(pk)) {
          return self.create(data, options, cb);
        } else if (modelsLength == 0 && data.hasOwnProperty(pk) == false) {
          cb("Error message: No matching instance found. Found error while trying to create a new instance.Primary key is not provided in the payload");
        } else if (modelsLength === 1) {
          var pkName = idName(Model);
          var idVal = instances[0][pkName];
          self.findById(idVal, function(err, modelInst) {
            if (err) {
              cb(err);
            } else {
              modelInst.updateAttributes(data, function(err, info) {
                if (err) {
                  cb(err);
                } else {
                  cb(null, info);
          ...
          cb("There are multiple instances found.Upsert Operation can't be performed ");
        }
      });
    } else {
      cb("data field can't be empty");
    }
  }
}
/**
